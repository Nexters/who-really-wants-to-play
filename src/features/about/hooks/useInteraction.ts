...
export const useIntroInteraction = (scrollValue: number, aboutContainerScrollY: number) => {
  ...
  const [profileBoxPaddingTop, setProfileBoxPaddingTop] = useState<number>(INTRO_SETTINGS.boxPaddingTop.startValue);
  ...
  const playIntroInteraction = () => {
    ...
    setProfileBoxPaddingTop(calcValue(INTRO_SETTINGS.boxPaddingTop));
  };
